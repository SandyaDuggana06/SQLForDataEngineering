...
select count(email) from customers where email ='Not Provided'

--Find the percentage of customers with missing emails.
SELECT
    COUNT(*) AS total_customers,
    COUNT(CASE WHEN email ='Not Provided' THEN 1 END) AS customers_with_missing_email,
    CAST(
        COUNT(CASE WHEN email ='Not Provided'  THEN 1 END) * 100.0
        / COUNT(*)
        AS DECIMAL(5,2)
